...
- [ ] **L21 — Non-fixed arrays:** frontend unblocked incrementally: `ArrayType::Variable(Symbol)` now preserves simple integral identifier bounds such as `int a[n]` instead of replacing them with `Fixed(1)`. Identifier runtime bounds now reach the backend. Next backend slice is dynamic stack storage/addressing; general runtime bound expressions, `sizeof(VLA)`, nested VLA shapes, and focused execution tests remain. Incomplete `T a[]` remains distinct.
