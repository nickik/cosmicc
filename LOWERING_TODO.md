...
- [ ] **L20 — Designated aggregate initialization:** frontend work started: AST now represents member/index designators, the parser accepts `.member =` / `[index] =`, and the analyzer normalizes single-level designators into positional HIR. Nested member/index designator paths are now recursively normalized. Sparse gaps now have an explicit `Initializer::Zero` HIR representation and SIA lowering preserves zero-filled storage. Continuation now resumes at the subobject following a designator for arrays and structs. Remaining before completion: override semantics and focused SIA local/global designated-initializer tests.
