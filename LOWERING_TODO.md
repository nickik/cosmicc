...
- [ ] **L21 — Non-fixed arrays:** frontend unblocked incrementally: `ArrayType::Variable(Symbol)` now preserves simple integral identifier bounds such as `int a[n]` instead of replacing them with `Fixed(1)`. Identifier runtime bounds reach the backend, and `FunctionLowerer` now has explicit VLA base-address plumbing integrated with local address formation. Dynamic stack allocation audit found no dynamic stack-slot/alloca primitive exposed by the pinned Cranelift/SIA32 path. L21 therefore requires an explicit runtime stack-pointer allocation mechanism in the backend before VLA declarations can be lowered correctly; fixed StackSlots must not be used as a semantic substitute. General runtime bound expressions, `sizeof(VLA)`, nested VLA shapes, and execution tests remain after that backend primitive. Incomplete `T a[]` remains distinct.
