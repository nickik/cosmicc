...
- [ ] **L21 — Non-fixed arrays:** frontend unblocked incrementally: `ArrayType::Variable(Symbol)` now preserves simple integral identifier bounds such as `int a[n]` instead of replacing them with `Fixed(1)`. Remaining: preserve general runtime bound expressions, implement dynamic SIA32 stack allocation/addressing, `sizeof(VLA)`, nested VLA shapes, and focused execution/lowering tests. Incomplete `T a[]` remains distinct.
