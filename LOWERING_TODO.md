...
- [ ] **L20 — Designated aggregate initialization:** frontend work started: AST now represents member/index designators, the parser accepts `.member =` / `[index] =`, and the analyzer normalizes single-level designators into positional HIR. Nested member/index designator paths are now recursively normalized. Remaining before completion: correct sparse zero-fill representation, continuation after a designator, override semantics, and focused SIA local/global lowering tests.
